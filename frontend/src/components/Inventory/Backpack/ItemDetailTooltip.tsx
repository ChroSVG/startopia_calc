...
import { Clock, Hand, Pickaxe, Box } from "lucide-react"
import { BackpackItem } from "./BackpackSlot"
...
export const ItemDetailTooltip = ({ item, children }: ItemDetailTooltipProps) => {
  return (
    <Popover>
      <PopoverTrigger asChild>
        {children}
      </PopoverTrigger>
      <PopoverContent className="w-80 p-0 overflow-hidden bg-card border-2 shadow-xl" side="right" sideOffset={10}>
        ...
        >
          <div className="flex justify-between items-start gap-2 relative z-10">
            <div className="space-y-1 min-w-0">
              <h4 className="font-bold text-lg leading-tight truncate">{item.item_name}</h4>
              <div className="flex flex-wrap gap-1.5">
                {item.item_rarity && (
                  <span className={cn("text-[10px] font-bold uppercase tracking-wider", getRarityTextColor(item.item_rarity))}>
                    {item.item_rarity}
                  </span>
                )}
                {item.item_type && (
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
             <StatItem icon={<Box className="size-3" />} label="Quantity" value={item.quantity} />
             {item.grow_time !== undefined && (
               <StatItem icon={<Clock className="size-3" />} label="Grow Time" value={`${item.grow_time}s`} />
             )}
             {item.hits_with_hand !== undefined && (
               <StatItem icon={<Hand className="size-3" />} label="Hand Hits" value={item.hits_with_hand} />
             )}
             {item.hits_with_pickaxe !== undefined && (
               <StatItem icon={<Pickaxe className="size-3" />} label="Pickaxe Hits" value={item.hits_with_pickaxe} />
             )}
          </div>
        </div>
